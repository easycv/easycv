...
def test_name():
    p = Pipeline([Noise(method='gaussian'), Noise(method='pepper'), Noise(method='s&p')])
...
def test_num_transforms():
    p = Pipeline([Noise(method='s&p'), Noise(method='pepper'), Noise(method='gaussian')])
...
def test_save():
    p = Pipeline([Noise(method='s&p', amount=0.1523), Noise(method='pepper'), Noise(method='gaussian', var=0.4)], name='test')
    p.save()
